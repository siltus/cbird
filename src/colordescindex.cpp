...
void ColorDescIndex::removeRecords(QSqlDatabase& db, const QVector<QString>& idBatches) const {
  QSqlQuery query(db);
  for (const QString& batch : idBatches)
    if (!query.exec("delete from color where media_id in (" + batch + ")")) SQL_FATAL(exec);
}
